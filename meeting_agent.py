...
# Streamlit app setup
st.set_page_config(
    page_title="AI Meeting Preparation Agent",
    page_icon="📋",
    layout="wide"
)
st.markdown("""
...
Generate executive-ready meeting briefs, industry insights, strategic agendas, and discussion points using AI-powered multi-agent collaboration.
...
st.sidebar.markdown("""
Configure the AI models and search service before generating the meeting brief.
""")
...
if gemini_api_key and serper_api_key:
    ...
    st.caption("Provide the meeting details to generate an AI-powered executive briefing.")
    ...
    with col2:
        meeting_objective = st.text_input(
            "Meeting Objective",
            placeholder="e.g. Discuss AI partnership opportunities"
        )
    ...
    if st.button("🚀 Generate Meeting Brief",use_container_width=True,type="primary"):
        with st.spinner("Analyzing company, researching industry, and preparing executive brief..."):
            result = meeting_prep_crew.kickoff()
        st.success("Meeting preparation completed successfully.")
        st.markdown("## Executive Meeting Brief")
        st.markdown(result)
    ...
    3. Click '🚀 Generate Meeting Brief' to generate your comprehensive meeting preparation package.
    ...
    st.info("""### Welcome Enter your **Gemini API Key** and **Serper API Key** in the sidebar to begin.
